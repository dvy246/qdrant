...
def validate_and_canonicalize(
    ...
    # Verify canonical round-trip integrity
    verify_mol = Chem.MolFromSmiles(canonical_smiles)
    if verify_mol is None or verify_mol.GetNumAtoms() != mol.GetNumAtoms():
        return None
...
print(f"{'Rank':<6}{'SMILES':<45}{'Fused':<10}{'Cosine':<10}{'Tanimoto':<10}{'MW':<10}{'LogP'}")
print("-" * 100)
...
def process_smiles(
    smiles_list: list[str],
    toxicity_scores: list[float] | None = None,
) -> list[dict]:
    ...
    for i, smi in enumerate(smiles_list):
        ...
        canonical = Chem.MolToSmiles(mol)
        verify_mol = Chem.MolFromSmiles(canonical)
        if verify_mol is None or verify_mol.GetNumAtoms() != mol.GetNumAtoms():
            print(f"  Skipping failed integrity check: {smi}")
            continue
        ...
        if toxicity_scores is not None:
            ...
            if not math.isfinite(score):
                raise ValueError(f"toxicity_score must be finite: {score}")
            payload["toxicity_score"] = float(score)
...
# ---------------------------------------------------------------
def main():
    # Sample drug molecules with toxicity scores
    raw_smiles = [
        "CC(=O)Oc1ccccc1C(=O)O",              # Aspirin
        "CC(C)Cc1ccc(cc1)C(C)C(=O)O",          # Ibuprofen
    ...
    toxicity_scores = [0.18, 0.15, 0.48, 0.37, 0.22, 0.31, 0.14, 0.72, 0.08, 0.61]
    
    print("Step 1: Processing SMILES...")
    molecules = process_smiles(raw_smiles, toxicity_scores=toxicity_scores)
    ...
    print(f"\n  {'Rank':<6}{'SMILES':<50}{'Fused':<10}{'Cosine':<10}{'Tanimoto':<10}")
    print(f"  {'-' * 86}")
    for i, hit in enumerate(hits, 1):
        print(f"  {i:<6}{hit['smiles']:<50}{hit['fused_score']:<10}{hit['score']:<10}{hit['tanimoto_score']:<10}")
...
Step 4: Searching...
  ...
  Rank  SMILES                                            Fused     Cosine    Tanimoto
  --------------------------------------------------------------------------------------
  1     CC(=O)Oc1ccccc1C(=O)O                             1.0000    1.0000    1.0000
  2     OC(=O)c1ccccc1O                                   0.7xxx    0.95xx    0.4xxx
  3     OC(=O)c1ccccc1                                    0.7xxx    0.93xx    0.3xxx
  ...
```
